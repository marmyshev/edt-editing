...
public class ProjectPathEditingService {
	...
	public boolean canEdit(IProject project, EObject eObject) {
		...
		try {
			return cache.getUriCache().get(resourceUri, () -> {
				IProjectFileSystemSupport fileSystemSupport = fileSystemSupportProvider
						.getProjectFileSystemSupport(project);
	...
	private class ProjectCache {
		...
		private final Cache<String, Boolean> uriCache = CacheBuilder.newBuilder()
